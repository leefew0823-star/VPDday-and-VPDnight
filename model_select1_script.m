...
%% step 2
fig_composite = figure('Position', [50, 50, 1000, 1200]); 

num_cols = 3;
num_rows = ceil(num_sites / num_cols); 
...
for i = 1:num_sites
    ...
    if Delta_AIC > 2
        ...
        summary_table{i+1, 8} = round(best_p1(1), 3); 
        summary_table{i+1, 9} = format_p(p_val_k1);   
        summary_table{i+1, 10} = round(best_k2, 3);   
        summary_table{i+1, 11} = format_p(p_val_k2);  
        ...
        summary_table{i+1, 12} = round(p_lin(1), 3);  
        summary_table{i+1, 13} = format_p(p_val_k);   
        y_universal = polyval(p_lin, universal_vpd);
    end
    
    export_data(:, i + 1) = y_universal;
    hold on; grid off;
    plot(x_sort, y_sort, '-', 'Color', [0.8, 0.8, 0.8], 'LineWidth', 0.5);
    plot(universal_vpd, y_universal, '-', 'Color', [0, 0.5, 0], 'LineWidth', 2);
    set(gca, 'FontSize', 9, 'LineWidth', 1.0);
    box on;
end
%% step 4
composite_fig_filename = fullfile(output_folder, 'AridSites_CompositeFit_4x3.png');

summary_filename = fullfile(output_folder, 'Model_Summary_Statistics_arid_day.xlsx');

exportgraphics(fig_composite, composite_fig_filename, 'Resolution', 300);
close(fig_composite); 
